...
@section('title', 'Edit Pengirim Barang')
@section('header', 'Edit Pengirim Barang')

@section('content')
<div class="px-4 sm:px-0">
    <div class="max-w-2xl">
        <form action="{{ route('admin.pengirim-barang.update', $pengirimBarang) }}" method="POST" class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
            @csrf
            @method('PUT')
            ...
            <div class="px-4 py-6 sm:p-8">
                <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8">
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <input type="text" name="nama" id="nama" value="{{ old('nama', $pengirimBarang->nama) }}" 
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <textarea name="alamat" id="alamat" rows="3" 
                                      class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6"
                                      required>{{ old('alamat', $pengirimBarang->alamat) }}</textarea>
                        </div>
                        @error('alamat')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <select name="negara" id="negara" 
                                ...
                                <option value="jp" {{ old('negara', $pengirimBarang->negara) === 'jp' ? 'selected' : '' }}>Japan</option>
                                <option value="sg" {{ old('negara', $pengirimBarang->negara) === 'sg' ? 'selected' : '' }}>Singapore</option>
                            </select>
                        </div>
                        @error('negara')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        <div class="flex items-center">
                            <input id="is_active" name="is_active" type="checkbox" 
                                   ...
                                   {{ old('is_active', $pengirimBarang->is_active) ? 'checked' : '' }}>
